...
export default function SignUp() {
    ...
    const validate = (email: string, password: string) => {
        ...
        if (!email.includes('@')) alert('@ is missing');
        // eslint-disable-next-line no-alert
        if (password.length < 8) return alert('Password length should be atleast 8');
    ...
    function handleSignUp() {
        if (validate(email, password)) {
            localStorage.setItem('user-data', JSON.stringify({ email, password }));
            navigate('/');
        }
    }
